...
namespace Ccoach\Classes\Utilities;
...
class UserManagement
{
     ...
     * @return \Ccoach\Entity\User|null
     */
    public function getCurrentUser()
    {
        ...
        $currentUser = $this->objectManager->getRepository('Ccoach\Entity\User')->findOneBy(array('userId' => $userId));
    ...
    {
        $allUsers = $this->objectManager->getRepository('Ccoach\Entity\User')->findBy(array(), array('points' => 'desc'));
        $numUsers = count($allUsers);
        for ($i = 0; $i < 5 - $numUsers; $i += 1) {
            array_push($allUsers, new \Ccoach\Entity\User());
        }
        return $allUsers;
    }
    ...
    {
        //$allUsers = $this->objectManager->getRepository('Ccoach\Entity\User')->findBy(array(), array('pointsThisWeek' => 'desc'));
        ...
        $allUsers = $this->objectManager->createQuery(
            'SELECT u FROM Ccoach\Entity\User u WHERE u.lastAccess > ' . $aWeekAgo->format('Y-m-d') .
    ...
    public function getQuestionTypes()
    {
        ...
        $query = $queryBuilder->select(array('qt'))
            ->from('Ccoach\Entity\QuestionType', 'qt')
    ...
    public function getUpdatedSkills()
    {
        ...
        if ($numQuestionTypes > $numSkills) {
            ...
            for ($questionIdentIndex = 0; $questionIdentIndex < $numQuestionTypes; $questionIdentIndex++) {
                ...
                if ($newSkill) {
                    $userSkill = new \Ccoach\Entity\UserSkill();
    ...
    public function addResults($results)
    {
        ...
        $currentSession = new \Ccoach\Entity\GameSession();
    ...
    public function updateWeekPoints()
    {
        ...
        $gameSessions = $this->objectManager->createQuery(
            'SELECT u FROM Ccoach\Entity\GameSession u WHERE u.userId =  ' .
